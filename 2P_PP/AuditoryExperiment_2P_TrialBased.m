...
mkdir(runfolder)
runfolderOT = fullfile(runfolder, 'originalTiffs');
mkdir(runfolderOT)
cd(runfolder)
...
metadata.random         = 1;
plotting                = 0;
metadata.stimulusPath =  'C:\Users\Paola\Dropbox\Data\stimSettings\aVLP_explorer';     %WED_full';  %JON_FINAL';
[metadata, stimuli, ALLstimuli] = stimulusManager(runfolder,metadata,plotting);
%% 2. decide stimulus composition 
% % 
clipboard('copy',[runtag, '_']);
metadata.ITI            = 0.7; %changed. Kept here to avoid errors in interpreter (for now)
ITIpercent              = 0.7;
% metadata.totalDur     = 9;
metadata.shuffleWithin  = 1;
metadata.startPadDur    = 1.8; %1.2800;
metadata.endPadDur      = 0.8;
metadata.fs             = 4e4;
metadata.maxVoltage     = 5; %use max 0.5 if powering a speaker!!
metadata.random         = 1;
plotting                = 0;
metadata.stimulusPath =  'C:\Users\Paola\Dropbox\Data\stimSettings\stim_FINAL'; %BRAIN_explorer';    %WED_full_nonOPT   %WED_full';  %JON_FINAL';
[metadata, stimuli, ALLstimuli] = stimulusManager(runfolder,metadata,plotting);
%% 2. decide stimulus composition 
% % 
clipboard('copy',[runtag, '_']);
...
% metadata.totalDur     = 9;
metadata.startPadDur    = 2; %1.2800;
metadata.endPadDur      = 2;
metadata.shuffleWithin  = 1;
metadata.fs             = 4e4;
metadata.maxVoltage     = 5; %use max 0.5 if powering a speaker!!
metadata.random         = 1;
plotting                = 0;
metadata.stimulusPath =  'C:\Users\Paola\Dropbox\Data\stimSettings\Exploratory_GC6S';
[metadata, stimuli, ALLstimuli] = stimulusManager(runfolder,metadata,plotting);

%%
% clear stimuli ALLstimuli
% stimuli = M;
% ALLstimuli = ST;
...
totTime = (1+ITIpercent) * metadata.totDurRun; 
...
if strcmp(readygo, 'Start')
    ...
    for t = 1:length(ALLstimuli)   
        ...
        stim = ALLstimuli(t).stim.stimulus;
        stimDur = ALLstimuli(t).stim.stimDur;
        ...
        queueOutputData(s,[stim, extTrig]); %check channel order and everything
    ...
    metadata.ITIpercent = ITIpercent;
